                ...
                <div class="widget-chart with-sidebar">
                    ...
                    <div class="widget-chart-sidebar bg-light doughnut-poe">
                        ...
                        <ul class="chart-legend fs-11px">
                            ...
                            <li class="text-dark actual"><i
                                    class="fa fa-circle fa-fw text-teal fs-9px me-5px t-minus-1"></i>
                                <span>Actual</span>
                            </li>
                        </ul>
                    </div>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content clearfix">
                <div class="modal-header">
                    <h4 class="modal-title">Scan Your QR</h4>
                </div>
...
@endsection
@push('script')
    <script src="{{ asset('assets/js/html5-qrcode.reader.js') }}"></script>
    <script>
        ...
        function onScanSuccess(decodedText, decodedResult) {
            if (decodedText !== lastResult) {
                ++countResults;
                let credentials = decodedText.split('|');
                $.ajax({
                    url: "{{ route('auth.qr.login') }}",
                    type: "POST",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        txtqrcode: credentials[1]
                    },
                    dataType: "JSON",
                    success: function(response){
                        $('#loginModal').modal('hide');
                        window.location.href = "{{ route('input.opr.index') }}";
                    },
        ...
        $(document).ready(function() {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('.doughnut-poe').find('.chart-number').html(response.poe.poe +
                        "% <small>Actual</small>");
