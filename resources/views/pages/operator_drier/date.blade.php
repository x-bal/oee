...
@section('title', 'Input Data OEE Drier')
...
@section('content')
    <div class="d-flex align-items-center mb-3">
        <div>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Input Data OEE Drier</li>
            </ol>
            <h1 class="page-header mb-0">Input Data OEE Drier</h1>
        </div>
    </div>
	<!-- BEGIN panel -->
	<div class="panel panel-inverse">
		...
		<div class="panel-heading">
			<h4 class="panel-title">Input Data OEE Drier</h4>
            ...
            <div class="table-responsive">
                <table id="daTable" class="table table-striped table-bordered align-middle">
                    ...
                    <tbody>
                        ...
                        @php
                            $i++;
                            $oee = (($val['ar']/100)*($val['pr']/100)*($val['qr']/100))*100;
                            $bg_striped = ($oee < 40?'bg-danger':($oee < 80?'bg-primary':'bg-success'));
                        @endphp
                        <tr>
                            <td>
                                <span class="alert alert-success"><i class="fas fa-calendar-alt"></i> {{ $val['dates'] }}</span>
                            </td>
                            <td align="middle">
                                <div class="progress progress-striped active">
                                    <div class="progress-bar {{ $bg_striped }} progress-bar-striped progress-bar-animated rounded-pill fs-10px fw-bold" style="width: {{ number_format($oee) }}%">OEE = {{ number_format($oee, 2) }}%</div>
                                </div>
                            </td>
                            <td align="middle"><i class="ion-md-analytics text-success"></i>
                                {{ $val['ar'] }}%
                            </td>
                            <td align="middle"><i class="ion-md-podium text-success"></i>
                                {{ $val['pr'] }}%
                            </td>
                            <td align="middle"><i class="ion-md-ribbon text-success"></i>
                                {{ $val['qr'] }}%</td>
                            <td>
                                {{ $val['worktime'] }}
                            </td>
                            <td class="text-center">
                                @foreach ($shift as $item)
                                    <button {{ (date('Y-m-d', strtotime($val['dates'])) > date('Y-m-d')?'disabled':'') }} data-shift="{{ $item->id }}" data-date="{{ date('Y-m-d', strtotime($val['dates'])) }}" class="btn btn-sm btn-success btn-shift" data-bs-toggle="modal" data-bs-target="#oeeModal"><i class="fas fa-file-excel"></i> {{ $item->txtshiftname }}</button>
                                @endforeach
                            </td>
                        </tr>
                        @endforeach
    ...
    <script>
        ...
        function getData(shift, date){
            ...
            $('#oeeModal').on('shown.bs.modal', function(e){
                $.ajax({
                    url: "{{ route('operator.drier.oee.list') }}",
                    type: "POST",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        shift: shift,
            ...
            Handsontable.dom.addEvent(save, 'click', () => {
                ...
                $.ajax({
                    url: "{{ route('operator.drier.oee.store') }}",
                    type: "POST",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        "datas": hot.getData()
            ...
            function okpDetail(row, okp){
                $.ajax({
                    url: "{{ route('operator.drier.oee.okp') }}",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        okp:okp
                    },
            ...
            function shiftAndDate(row, shift, date){
                ...
                hot.setDataAtRowProp(row, 'operator', "{{ Auth::user()->txtinitial." - ".$leader->txtinitial }}");
            }
