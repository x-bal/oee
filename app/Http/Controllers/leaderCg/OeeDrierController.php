...
namespace App\Http\Controllers\leaderCg;
...
class OeeDrierController extends Controller
{
    ...
    {
        $this->columns = ['OEE ID', 'Line ID', 'LINE NAME','Shift', 'Tanggal', 'Start', 'Finish', 'Lama Kejadian (Min)', 'Activity_id', 'Activity Code', 'Deskripsi Kejadian','Remarks', 'Operator', 'OKP Drier', 'Art Code', 'Nama Produk', 'Waiting Technician (min)', 'Technician Name', 'Repair Problem (min)', 'Start up/Trial Test (min)', 'Category Breakdown 8 Basic Competency', 'Category Breakdown 4 M', 'Category Breakdown AM/PM', 'Output (BIN)', 'Output (KG)', 'Rework (kg)', 'Category Rework', 'Reject (kg)', 'Jumlah Manpower'];
    }
    public function getMonth(Request $request)
    {
        ...
        $line_selected = ($request->has('line_id')?$request->line_id:$line[0]['id']);
        $loop = [];
        ...
        foreach (range(1, 12) as $key => $val) {
            ...
            $loop[] = [
                'link' =>
                    'month-drier/' . date('Ym', mktime(0, 0, 0, $val, 5, $nowyear)).$line_selected,
        ...
        // die();
        return view('pages.leader-cg.month-drier', [
            'line' => $line,
            'year' => $year,
            'oee' => $loop,
        ]);
    ...
    public function getDate($month)
    {
        ...
        return view('pages.leader-cg.date-drier', [
            'dates' => $loop,
            'code' => $code,
    ...
    public function getDateList($month)
    {
        ...
        foreach (range(1, date('t', $date)) as $key => $val) {
            ...
            $dmY = $val . '-' . substr($month, 4, 2) . '-' . substr($month, 0, 4);
